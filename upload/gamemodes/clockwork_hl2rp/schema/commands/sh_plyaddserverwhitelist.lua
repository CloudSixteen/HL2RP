...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		if (target:GetData("ServerWhitelist")[identity]) then
			Clockwork.player:Notify(player, {"AlreadyOnServerWhitelist", target:Name(), identity});
		...
		Clockwork.player:NotifyAll({"AddedToServerWhitelist", player:Name(), target:Name(), identity});
	else
